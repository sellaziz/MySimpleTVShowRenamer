...
from renamer.search_tv_show import get_ep_list
from renamer.search_tv_show import get_best_matched_name
...
def main1(dirname):
    ...
    for best_match in final_dict.keys():
        ...
        for filename in final_dict[best_match].keys():
            ...
            print(filename.ljust(65), "-->",
                  final_dict[best_match][filename]['new_name'].ljust(75))
...
def find_rename(filename_list):
    ...
    # Associate Best Match with filenames
        for season in seasons.keys():
            best_match = sort_dict[key]
            ep_dicts.setdefault(best_match, {})
            ep_dicts[best_match].setdefault(
                season, get_ep_list(best_match, season))
        if debug:
            print("Show : {0} and seasons: {1}".format(
